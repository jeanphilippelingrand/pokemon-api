...
describe.only('PokemonService', () => {
  ...
  describe('findAll', () => {
    ...
    it('Should throw RepositoryError if prisma throw an unhandled error', async () => {
      mockPrismaService.pokemon.findMany = jest
        .fn()
        .mockRejectedValue({ code: 1, cause: 'Fake error cause' });
      let error;
      try {
  ...
  describe('Create', () => {
    it('Should create a pokemon with prisma', async () => {
      mockPrismaService.pokemon.create = jest
        .fn()
    ...
    it('Should throw PokemonAlreadyExistsError when prisma throws Unique constraint error', async () => {
      mockPrismaService.pokemon.create = jest
        .fn()
        .mockRejectedValue({ code: PrismaService.UNIQUE_CONTRAINT_ERROR_CODE });
